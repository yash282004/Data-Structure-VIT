...
To write a C++ program to accept a graph from the user, represent it using an adjacency matrix, and perform Breadth-First Search (BFS) and Depth-First Search (DFS) traversals.

## Problem Statement:

Create a graph with user input and represent it using an adjacency matrix.
Perform and display:
...
The graph may be directed or undirected (your choice).
Traversal must begin from a user-entered starting vertex.
...
- Create an adjacency matrix n × n and initialize all values to 0.

4. Step 4:

- Accept edges from the user and fill the matrix:
...
- Visit all neighbors level-wise
...
- The user enters vertices and edges.

- DFS uses a stack to go deep into nodes.

- BFS uses a queue to visit level-wise.

- The matrix and traversals are printed clearly.